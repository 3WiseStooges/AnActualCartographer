...
world three times as far as you used to, for as long as that character plays that world.

## How it works

Build a cartography table and interact with it — either switch, "read map" or "save map".
That is the whole unlock. From then on the fog lifts **384m** around you instead of the
usual **128m**.
...
## Why 128m and 384m, when vanilla says 100m?
...
This mod counts in the same pixels the game does, so `ExploreRadiusMultiplier = 3` clears
6 pixels — 384m, exactly three times as far. Scaling the raw 100m instead would have landed
on 320m, which is only 2.5x, and the config would have been quietly lying to you.
...
| `ExploreRadiusMultiplier` | `3` | How much further the fog lifts after the table. `3` = three times as far (384m). Range 1–50. |
